...
tasks = generate_tasks(
    resources=resources, task_count=num_tasks, ratio=0.5, utilization_ub=5
)
num_resources = 3
resources = generate_resources(resource_count=num_resources)
...
for speedup_factor in speedup_factor_list:
    cedf = CriticallyEDF(
        tasks=tasks, resources=resources, speedup_factor=speedup_factor, verbose=False, overrun_chance=30
    )
    if cedf.schedule():
        cedf.visualize(
            show=False, save=True, title=f"Critically EDF - Speedup Factor {speedup_factor}", filename=f"overrun-speedup{speedup_factor}"
        )
...
fig.savefig("overrun-speedup-factor-quality-of-service.png")
...
fig.savefig("overrun-speedup-factor-missed-deadline-count.png")
